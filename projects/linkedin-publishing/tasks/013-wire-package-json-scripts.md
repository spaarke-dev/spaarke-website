# Task 013: Wire `pnpm linkedin:*` scripts in package.json
...
Operator runs `pnpm linkedin:auth`, `pnpm linkedin:publish`,
`pnpm linkedin:test-kv` directly without remembering `tsx` paths.

## Context

The skill in task 012 shells out to these pnpm scripts. Each one
...
2. Under `"scripts"`, add (alphabetically grouped with existing scripts):
   ...
   ```
3. The `linkedin:status` script doesn't exist yet (task 040) — that's OK, the entry can be wired now since pnpm only resolves it when invoked.
...
- [ ] `pnpm linkedin:test-kv` prints `KV reachable, client-id length: 14`.
- [ ] `pnpm linkedin:auth --help` (or `--app=member`) launches the auth flow (assumes task 010 done).
- [ ] `pnpm linkedin:publish --help` (or `--slug=foo --target=personal --dry-run`) shows usage or runs dry (assumes task 011 done).
