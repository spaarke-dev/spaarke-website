...
Rename the route folder `src/app/blog/` → `src/app/insights/` and update internal references. Add a permanent redirect `/blog` → `/insights` (and `/why-spaarke/[slug]` → `/insights/[slug]`) via `next.config.ts` redirects, so existing links and SEO juice survive. Update sitemap to use the new path. Update RSS feed URL.
...
- [ ] Permanent redirect from `/blog` and `/why-spaarke/[slug]` works
