# What Is Ontology (Preview)? - Microsoft Fabric | Microsoft Learn

> **Source:** Microsoft Learn — https://learn.microsoft.com/en-us/fabric/iq/ontology/overview
> (source file `docs/iq/ontology/overview.md` in `MicrosoftDocs/fabric-docs`).
> Saved 2026-09-21 as reference material for the `legal-operations-ontology` article.
> © Microsoft. Reproduced under [CC BY 4.0](https://creativecommons.org/licenses/by/4.0/),
> the license of the fabric-docs repository. Formatting was altered by the
> save-to-markdown conversion; relative links do not resolve here. Cite the
> Microsoft Learn URL, not this copy.

The *ontology (preview)* item (part of the [Fabric IQ (preview) workload](../overview)) digitally represents the enterprise vocabulary and semantic layer that unifies meaning across domains and OneLake sources. It defines enterprise concepts as *entity types* (like *Customer*), *properties* (like a Customer's *name* and *email*), and *relationships* (like *Customer places Order*), while clarifying the constraints of these terms. After defining your ontology, bind the entity type definitions to real data, so downstream tools can share the same language. Both humans and AI agents can use this language for cross-domain reasoning and decision-ready actions.
...
After you define an ontology, you can [bind it to your data](how-to-bind-data) to visualize and query the data in the context of your ontology. Read about each core data concept in the following sections.
...
Any updates in upstream data sources (like new rows) need to be manually refreshed before they're visible in the ontology item. For more information, see [refresh the graph model](how-to-view-entity-type-details#refresh-the-graph-model).
