...
For a series, the writer sets a target. His calibration after rewriting the Legal Operations Intelligence series is 4,300 to 5,500 words and 8 to 11 sections. Treat that as the number to aim at and the number to justify departing from, not as a cap: of the five finished articles, three land inside it and two do not, the platform piece at 5,621 words and the ontology piece at 6,245 across 12 sections, both because the subject needed the room. What the range is really guarding against is the first-draft failure it was derived from, which was 13 to 19 thin sections that restated each other. No section should restate another, and a section far heavier than its neighbours is usually holding an inventory that belongs distributed into the argument.
...
**`summary`**: two to three sentences, with a target of 150 to 280 characters and a maximum of about 300. It renders as the article preview on the `/why-spaarke` index card and on the article-page hero. It should be distinct from `description` (the SEO meta description): it is written as narrative and is not stuffed with keywords. The `keyTakeaways` carry the article's positions, and the job of `summary` is to give the reader a reason to open the article. For a long-form article the writer's ruling of 2026-09-23 is that the summary may run past that range: it is the on-page "Why this matters" block, where the length is doing work, and the five articles of the Legal Operations Intelligence series run 277 to 538 characters. The `description` field, not `summary`, supplies the meta description, the Open Graph description and the Twitter card, so a long summary costs nothing in search results or link previews. Keep `description` inside 120 to 160 characters for that reason.
...
A link to a Spaarke article is normally introduced in the citing first person, with Spaarke as the actor: "In our earlier article, [title], we discussed X." The exception is a sentence whose subject is the problem rather than our treatment of it, where a bare link is correct ("an old problem, described in [Institutional Knowledge Is Walking Out the Door]"); a possessive frame that makes the article the actor and a third-person frame that makes Spaarke the actor are also house form, and none of the three is rewritten in a sweep (`voice/style-guide.md`, section 1). A link to a `/platform/<section>` page takes the ordinary form, because a capability page is not an authored argument. Note that `/why-spaarke/<slug>` is the route our articles publish to, so a link of that shape is a link to one of our articles and takes the citing form. The distinction is how a reader tells our body of work from the field's research, and a long-form article in a series should read as the next instalment of a position we have been developing rather than as a survey of other people's findings.
