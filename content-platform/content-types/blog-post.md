...
1. **Opening.** Build the opening on a situation, a complication, and an answer. The situation is something the reader already accepts, the complication is what has changed or what is going wrong, and the answer is the thesis. State the thesis within the first 250 words and no later than the third paragraph, in one sentence that the reader could repeat. The style guide's general target of 150 words still applies wherever the situation and the complication can be stated briefly, so 250 words is the outer limit for an opening that needs a fuller situation. The openers ruled out by `style-guide.md`, section 5, rule 3, remain ruled out. Two model openings are in `consulting-register.md`, section 6.
2. **Sections.** Use as many H2 sections as the argument needs. Each H2 heading carries a message: it states the point of its section in sentence case, typically in 4 to 12 words, so that a reader who reads only the headings can follow the argument. Each section opens with a sentence or two that connect it to what the previous section established, and it neither announces itself nor recaps itself. H3 subheadings are allowed where a section has distinct parts, and they follow the same rule. Where the opening previews a counted framework ("three decisions"), the same labels return in the same order as headings.
...
4. **Exhibits.** Refer to each exhibit in parentheses at the end of the sentence that states the finding, in the form "(Exhibit 1)". The exhibit's own title states the finding as a full sentence, and the text never says that a chart "below" shows something.
5. **Close.** The close ends on consequence. The final paragraph states what follows from the argument for the reader: a decision to make, a risk that grows, or a question that remains open. It does not summarize the article, it carries no "Conclusion" heading, and it does not pitch. Related reading may follow as a plain list of links (section 5).
...
2. **Three to five H2 sections.** Each section has its own sub-claim and at least one piece of named evidence (a number, a framework, or a concrete scenario). Headings are sentence-case statements of the point of the section. Question headings are governed by `style-guide.md`, section 5, rule 2.
...
- **Paragraphs**: a topic sentence, development, and evidence, typically in 50 to 100 words. One-sentence paragraphs are functional only (a lead-in to a list, a block quotation, or an exhibit).
...
The article ends on its final paragraph, and that paragraph ends on consequence (`style-guide.md`, section 5, rule 4). Two elements may follow it, and both sit outside the argument.
...
**`summary`**: two to three sentences, with a target of 150 to 280 characters and a maximum of about 300. It renders as the article preview on the `/why-spaarke` index card and on the article-page hero. It should be distinct from `description` (the SEO meta description): it is written as narrative and is not stuffed with keywords. The `keyTakeaways` carry the comprehensive summary, and the job of `summary` is to give the reader a reason to open the article.

**`keyTakeaways`**: an array of 3 to 5 strings of 18 to 35 words each, drafted in Spaarke voice. They appear in the "Key takeaways" card above the body.
...
- **Padding a long-form article.** Having no cap does not make length a goal. Restating the thesis in each section, in fresh words, is the most common form of padding (`ai-tells.md`, section 3.7).
- **The marketing close.** A close that begins `If you are ready to transform your legal department` breaks `style-guide.md`, section 5, rules 4 and 6. The close ends on consequence.
