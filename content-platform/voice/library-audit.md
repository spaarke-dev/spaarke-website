...
> **Note added 2026-09-21: superseded in part.** This audit is a historical record from May 2026, and its findings are unchanged. Its praise of the following devices is superseded by the 2026-09-21 revision of `style-guide.md`, which now prohibits them:
>
> - two-noun framings and `X, not Y` contrasts (strength 2, and the first item under "Voice elements");
> - short sentences used as landings, and one-sentence paragraphs used as hinges or for emphasis (strength 7);
> - bold lead-in bullets that carry the argument (strength 8);
> - negation followed by a diagnosis, including the "direct provocation" cited in strength 1;
> - the "Where to Go Next" close that restates the article (strength 6);
> - the em dash landings, the escalating three-sentence reveal, and the "controlled-rhythm" close that are recommended as extracts under "Recommended source material".
>
> The calibration line's references to the Stripe blog and Microsoft WorkLab no longer apply either. Where this audit conflicts with the style guide, the style guide governs. The reasons for the revision are recorded in `content-platform/research/2026-09-loi-series/notes/library-audit-local.md`. The em dashes in this file were removed on the same date without any change to the findings, and where a quoted passage contained one, the placeholder [EM DASH] marks the position.

**Phase**: 0 (T01)
**Scope**: every `.mdx` file in `content/blog/` as of 2026-05-06
**Calibration**: T00 §1 (McKinsey Quarterly, HBR, Stripe blog, Microsoft WorkLab/Fabric, BCG, Thomson Reuters Institute). Operator-grade, structured, plainspoken, analytically defensible, and closer to McKinsey/HBR than to startup marketing.
...
3. **Defends category claims with named external frameworks, not vibes.** The library cites by name: CLOC Core 12, ACC Maturity Model 2.0, Gartner's Legal Operations Maturity Model, SimpleLegal/Onit, Wolters Kluwer's 94% data-difficulty stat, the Center for American Progress 100% to 213% replacement-cost figure, the >50% Copilot-adoption stat. This is the McKinsey/HBR move of grounding new vocabulary inside the existing professional landscape rather than asserting a new category from outside it.
...
- **Why included**: Cleanest example of the category-definition move: it opens with a concrete scene, names the structural problem, introduces the new vocabulary, distinguishes it from adjacent categories, and earns the close. If a reader has time for one piece, this is the one that defines the voice.
- **Suggested extracts**:
  - Opening (paragraphs 1 to 4, "Consider the legal department of a Fortune 500…" through "And it has a name."): concrete-scene opener and patient setup before naming the new term.
...
- **Suggested extracts**:
  ...
  - "Why Tools Alone Will Not Close It" opener ("An AI tool layered on fragmented data gives you fragmented answers [EM DASH] faster."): characteristic single-sentence move, a counterintuitive operational truth in one breath.
...
*Locked 2026-05-07. Supersession note added and em dashes removed 2026-09-21; see git log for history.*
