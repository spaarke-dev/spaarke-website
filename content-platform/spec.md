...
│   ├── blog-post/
│   │   ├── brief.md                     ← brief template (the spec shape)
│   │   ├── idea.md                      ← idea-stage prompts
...
│   ├── white-paper/
│   └── tweet/
├── articles/                            ← per-piece workspaces (actual pieces; one dir per piece)
│   └── <slug>/                          ← the content equivalent of projects/<name>/
│       ├── idea.md                      ← writer's raw idea
│       ├── brief.md                     ← canonical brief (generated by idea-to-brief)
│       ├── plan.md                      ← structural outline (generated by content-pipeline)
│       ├── tasks.md                     ← workflow gates (generated by content-pipeline)
│       ├── CLAUDE.md                    ← per-piece router (generated by content-pipeline)
│       └── draft.{mdx|md}               ← the working draft (moves to publish target on ship)
├── published/                           ← finalized linkedin posts, tweets, papers (manual posting)
│   ├── linkedin-posts/
│   ├── tweets/
│   └── white-papers/
├── campaigns/                           ← coordinated multi-asset pushes around a theme
│   ├── README.md
│   ├── _template-campaign.md
│   └── <YYYY-MM>-<slug>.md
├── calendar.md                          ← cadence record (with GitHub Project cross-reference)
└── github-setup.md                      ← GitHub Project + Issues + Milestones reference
```

The per-piece workspace mirrors the software project pattern:
software has `projects/<name>/{idea.md, spec.md, plan.md, tasks/,
CLAUDE.md}`; content has
`content-platform/articles/<slug>/{idea.md, brief.md, plan.md,
tasks.md, CLAUDE.md, draft.mdx}`. Skills `idea-to-brief` and
`content-pipeline` (in `.claude/skills/`) generate these
artifacts and gate the workflow with human-in-loop confirmations.
...
For per-piece work, the per-piece `articles/<slug>/CLAUDE.md` is
authoritative — it specifies the exact load order for that piece.
For cross-piece work (editing voice docs, designing a campaign),
or when starting from scratch, the order is:
...
6. `articles/<slug>/brief.md` — the article-specific brief.
...
with shared sections. Stored at `articles/<slug>/brief.md`.
...
| **Brief** | Team + `idea-to-brief` skill | `articles/<slug>/brief.md` filled in |
| **Outline** | Claude | Section structure + key claims; no prose yet. Reviewed before drafting. |
| **Draft** | Claude | Full draft in `articles/<slug>/draft.{mdx|md}` |
